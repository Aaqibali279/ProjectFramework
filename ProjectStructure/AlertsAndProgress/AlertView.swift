...
class AlertView: UIView {
    ...
    private func setDoubleActions(){
        ...
        action = button(actionTitle, .redColor)
    ...
    private func setCancelButton(){
        cancel = button(Constants.CANCEL, .white)
    ...
    private var messageLabel = { () -> Label in
        let label = CustomLabel(textAlignment: .center, fontType: .regular, fontSize: FontSize.sub_title_22)
        return label
    }()
    
    private var headingLabel = { () -> Label in
        let label = CustomLabel(textAlignment: .center, fontType: FontType.demi_bold, fontSize: FontSize.sub_title_25)
        return label
    }()
